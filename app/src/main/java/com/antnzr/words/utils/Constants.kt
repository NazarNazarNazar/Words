package com.antnzr.words.utils

const val GOOGLE_TRANSLATE_ACTION = "google_translate_action"
const val CONTEXT_REVERSO_ACTION = "context_reverso_action"
const val PREVIOUS_WORD_ACTION = "previous_word_action"
const val NEXT_WORD_ACTION = "next_word_action"
const val LIST_ACTION = "list_action"

const val TOKENS_SIZE = 4

const val LANG_FROM_INDEX = 0
const val LANG_TO_INDEX = 1
const val FROM_INDEX = 2
const val TO_INDEX = 3

const val SHARED_PREF = "prefs"
const val CURRENT_WORD = "currentWord"

const val WEB_RESOURCE = "web_resource"
const val GOOGLE_TRANSLATE = "translate.google.com"
const val CONTEXT_REVERSO = "context.reverso.net"
const val WORD_NEED_DETAILS = "word_need_details"

const val SUB_DOWNLOAD_DIR = "subDownloadDir"
const val SUB_LOCAL_DIR = "subLocalDir"

const val SUBTITLE_EXTENSION = ".srt"

